...
const count = (target, input) => {
  let count = 0;
  input.forEach((val) => {
    let subArrCount = val.reduce((acc, curr) => {
      if (curr === target) acc++;
      return acc;
    }, 0);
    count += subArrCount;
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map((val) => {
    let filteredArray = val.filter((val) => {
      return val % 5 === 0 && typeof val === 'number';
    });
...
let findMaleAndFemale = (data) => {
  return data.filter((val) => val.gender === 'male' || val.gender === 'female')
    .map((val) => val.name).join(' and ');
}

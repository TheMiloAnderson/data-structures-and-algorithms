...
const countNumberOfChildren = (arr) => {
  return arr.reduce((acc, curr) => {
    return acc + (curr.children ? curr.children.length : 0);
  }, 0);
};
...
const extractChildren = (arr) => {
  const aChars = arr.filter((val) => {
    return /a/i.test(val.name);
  });
  return aChars.reduce((acc, curr) => {
    if (curr.children) {
      curr.children.forEach((val) => {
        acc.push(val);
      });
    }
    return acc;

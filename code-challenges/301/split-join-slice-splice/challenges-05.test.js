...
const listFoods = (recipe) => {
  // I cannot think of any solution to this that uses slice() but does not use split()
  let result = [];
  recipe.ingredients.forEach((ingredient) => {
    let food = ingredient.replace(/[0-9]+ [a-z-]+ /, '');
    result.push(food);
  });
  return result;
}
...
const extractVowels = (str) => {
  ...
  for (let i = 0; i < letters.length; i++) {
    if (vowels.includes(letters[i])) {
      extractedVowels.push(letters[i]);
    } else {
      extractedLetters.push(letters[i]);
    };
  };
  extractedVowels.sort((a, b) => {
    return a > b;
  });
  return [extractedLetters.join(''), extractedVowels.join('')];
};

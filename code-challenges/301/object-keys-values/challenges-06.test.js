...
const hasChildrenValues = (arr, character) => {
  for (let val in arr) {
	  if (arr[val].name === character) {
	    return arr[val].children.length ? true : false;
    }
  }
};
...
const hasChildrenEntries = (arr, character) => {
  ...
  Object.values(arr).forEach((val) => {
    if (val.name === character) {
      result = val.children.length ? true : false;
    }
  });
  return result;
};
...
const houseSize = (arr) => {
  const sizes = [];
  for (let val in arr) {
    let house = arr[val].house;
    let members = 1;
    if (arr[val].spouse) members++;
    members += arr[val].children.length;
...
const houseSurvivors = (arr) => {
  ...
  for (let val in arr) {
    let house = arr[val].house;
    let members = 1;
    if (arr[val].spouse && !deceasedSpouses.includes(arr[val].spouse)) members++;
    members += arr[val].children.length;

...
const toTitleCase = (arr) => {
  return arr.map((val) => {
    return val.replace(/^[a-zA-Z]/, val.charAt(0).toUpperCase());
  });
};
...
let biggerThanLuke = (arr) => {
  const luke = arr.find((val) => val.name === 'Luke Skywalker');
  return arr.filter((val) => {
    return parseInt(val.mass) > parseInt(luke.mass);
...
------------------------------------------------------------------------------------------------ */
function helpCheck(board, row1, col1, row2, col2, row3, col3) {
  let win = false;
...
const detectTicTacToeWin = (board) => {
  let win = false;
  if (helpCheck(board, 0, 0, 1, 0, 2, 0)) win = true;
  if (helpCheck(board, 0, 1, 1, 1, 2, 1)) win = true;
  if (helpCheck(board, 0, 2, 1, 2, 2, 2)) win = true;
  if (helpCheck(board, 0, 0, 0, 1, 0, 2)) win = true;
  if (helpCheck(board, 1, 0, 1, 1, 1, 2)) win = true;
  if (helpCheck(board, 2, 0, 2, 1, 2, 2)) win = true;
  if (helpCheck(board, 0, 0, 1, 1, 2, 2)) win = true;
  if (helpCheck(board, 0, 2, 1, 1, 2, 0)) win = true;
  return win;
}
